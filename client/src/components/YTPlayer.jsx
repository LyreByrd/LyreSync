...
class YTPlayer extends React.Component {
  ...
  onPlayerReady() {
    this.socket = io(`https://${HOME_URL}:${SOCKET_PORT}`, {secure: true});
    // this.feedSocket = io(`http://${HOME_URL}:8080`);
    this.socket.on('initPing', () => {
      this.socket.emit('getClientActions', {
        host: this.props.sessionHost,
        service: 'youtube',
      });
    });
      ...
      if (event.type === 'stateChange') {
        ...
        if (newVideo !== currVideo) {
          this.player.loadVideoById({
            videoId: event.newVideo,
            startSeconds: event.newTime,
          });
        } else if (Math.abs(event.newTime - this.player.getCurrentTime()) > 1) {
          //console.log('time is wrong');
          if (
            this.player.getPlayerState() === 5 &&
            this.player.cuedTime &&
            Math.abs(event.newTime - this.player.cuedTime) <= 1
          ) {
